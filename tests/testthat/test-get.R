test_that("get_model_name", {
  ...
  # empty argument
  expect_equal(get_model_name(list()), list())
  # single argument
  expect_equal(get_model_name(s1), s1@name)
  expect_equal(get_model_name(p3), p3@model)
  # vectorized arguments
  expect_equal(get_model_name(c(s1,s2)), c(s1@name, s2@name))
  expect_equal(get_model_name(c(s1,p3)), c(s1@name, p3@model))
  # invalid arguments
  expect_error(get_model_name(assessment()))
})
